...
CREATE OR REPLACE FUNCTION public.client_decide_proposta(_token text, _proposta_id uuid, _decisao text, _motivo text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE _prop public.propostas%ROWTYPE; _cli public.leads%ROWTYPE; _loj public.lojistas%ROWTYPE;
BEGIN
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'lead_id', _cli.id,
    'proposta_id', _prop.id,
    'lojista_id', _prop.lojista_id,
...
CREATE OR REPLACE FUNCTION public.client_negociar_proposta(_token text, _proposta_id uuid, _preco numeric, _mensagem text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- Update admin_lead_full_history to include email events from email_send_log
...
BEGIN
  ...
  prop AS (
    SELECT lojista_id, MIN(created_at) AS proposta_at
  ...
  WITH events AS (
    ...
    UNION ALL
    SELECT esl.created_at,
           CASE WHEN esl.template_name = 'proposta-respondida' THEN 'email_proposta_respondida_stand'
                ELSE 'email_negociacao_stand' END,
           jsonb_build_object(
             'template', esl.template_name,
             'recipient', esl.recipient_email,
             'status', esl.status,
             'error', esl.error_message,
             'lojista_id', esl.metadata->>'target_lojista_id',
             'empresa', (SELECT empresa FROM public.lojistas WHERE id::text = esl.metadata->>'target_lojista_id')
           )
    FROM public.email_send_log esl
    WHERE esl.template_name IN ('proposta-respondida','contraproposta-cliente')
      AND (esl.metadata->>'lead_id') = _lead_id::text
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
